 ...
 *
 * This file intentionally contains only the browser bridge client now. The old
 * Hub engine calls were removed so this bundle can run outside the original
 * workstation.
 */
const TIMEOUT_MS = 30_000;

/** 把 unknown 收窄成 Record<string, unknown>;不是对象时返回空对象。 */
...
// ---------------------------------------------------------------------------
// browser-bridge (127.0.0.1:9302) 客户端 — 只 browser-profile-pool plugin 用
// ---------------------------------------------------------------------------
...
function resolveBridgeToken(): string | null {
  const fromEnv = process.env.BROWSER_BRIDGE_TOKEN ?? process.env.OPENCLAW_GATEWAY_TOKEN;
