 ...
 * frames. Replaces the narrow traffic view from OpenClaw's bridge /requests
 * endpoint (which only exposes URL/method/status/resourceType).
 *
 ...
 *
 * Lifecycle:
 *  - On startup: start a reconnect loop per known profile
 *  - When a CDP endpoint is up: attach to page target, enable Network domain
 *  - On browser exit: mark disconnected; loop keeps trying
 */
...
type CdpEndpoint =
  | { kind: "port"; cdpPort: number; label: string }
  | { kind: "url"; cdpUrl: string; label: string };
...
class TrafficCapture {
  ...
  async loadProfiles(): Promise<void> {
    try {
      const cfgRaw = (await readFile(OPENCLAW_CONFIG_PATH, "utf-8")).replace(/^\uFEFF/, "");
      const cfg = JSON.parse(cfgRaw) as { browser?: { profiles?: Record<string, { cdpPort?: number; cdpUrl?: string }> } };
      ...
      for (const [name, p] of Object.entries(profiles)) {
        ...
        if (typeof p.cdpUrl === "string" && p.cdpUrl.trim()) {
          ...
          endpoint = { kind: "url", cdpUrl, label: cdpUrl };
        } else if (typeof p.cdpPort === "number") {
          endpoint = { kind: "port", cdpPort: p.cdpPort, label: String(p.cdpPort) };
        }
  ...
  // Symbol. OpenClaw re-registers plugins for every subagent — each new
  // instance would get its own Map otherwise, and 7 concurrent instances on
  // a 5s reconnect tick produced ~70 log lines/min despite the 60s throttle.
  ...
  private async attachTarget(name: string, endpoint: CdpEndpoint, target: CdpTarget): Promise<void> {
    ...
    (c.Network["loadingFinished"] as (cb: (e: { requestId: string }) => void) => void)(async ({ requestId }) => {
      try {
        ...
        // body not available (redirects, preflight)
      }
    });
  ...
  getFetchInterceptPaused(profile: string): Array<{ requestId: string; url: string; method: string; headers: Record<string, string>; postData?: string; capturedAt: string; timerRemainingMs: number }> {
    const p = this.profiles.get(profile);
    if (!p) return [];
    const result: Array<{ requestId: string; url: string; method: string; headers: Record<string, string>; postData?: string; capturedAt: string; timerRemainingMs: number }> = [];
  ...
  async fetchContinue(profile: string, capturedRequestId: string, headerOverrides?: Record<string, string>): Promise<{ ok: boolean; error?: string }> {
    const p = this.profiles.get(profile);
    ...
    p.fetchInterceptState.paused.delete(capturedRequestId);
    ...
    try {
      await (client.Fetch["continueRequest"] as (q: unknown) => Promise<unknown>)({
        requestId: entry.requestId,
        ...(headerOverrides ? { headers: Object.entries(headerOverrides).map(([name, value]) => ({ name, value })) } : {}),
      });
      return { ok: true };
    } catch (err) {
  ...
  /** Spool evicted ring-buffer entries to disk (~/.openclaw/cdp-spool/<profile>/<date>.jsonl).
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    // ─── cdp_get(type, id) — dispatches 2 single-entry fetchers ───
    api.registerTool((_ctx) => defineCdpTool({
      name: "cdp_get",
      label: "获取单条 CDP 详情",
      description: "按 type + id 获取单条完整 CDP 条目。type=traffic_detail 按 requestId 拿完整流量（headers/initiator/WS/SSE/redirectChain，body 在 bodyPath 用 read_file 读）。type=script_source 按 scriptId 拿浏览器编译后 JS 源码。",
      details: "合并 cdp_traffic_detail + cdp_script_source 为 type-based 工具。traffic_detail：按 requestId（来自 cdp_query type=traffic 或 fetch 返回的 requestId）获取单条完整 Network entry，含 request/response headers、完整调用栈 initiator、redirectChain、WebSocket frames/lifecycle、SSE messages。script_source：按 scriptId（来自 cdp_query type=scripts 返回的 scriptId）调用 Debugger.getScriptSource 获取浏览器编译后的 JS 源码。注意：body 不在 JSON 返回中而落盘到 bodyPath；需 profile 有活跃 CDP 客户端。",
      schema: CdpGetSchema,
      async execute(params) {
        if (params.type === "traffic_detail") {
          const ps = _capture.getProfile(params.profile);
          const buf = ps?.store ?? [];
          const entry = buf.find((r: any) => r.requestId === params.id)
            ?? ps?.fetchPaused?.find((r: any) => r.requestId === params.id);
          if (!entry) return { error: "request_not_found", requestId: params.id, hint: "requestId 可能已 ring buffer 滚出或不在该 profile" };
          return {
            requestId: entry.requestId,
            ...
            ws: entry.ws,
            wsLifecycle: entry.wsLifecycle,
            sse: entry.sse,
            hint: "body 不在 JSON 返回中。如需看 body 用 read_file 读 bodyPath 指向的文件。",
          };
        } else {
          const result = await _capture.getScriptSource(params.profile, params.id);
          return result;
        }
      },
    }) as AnyAgentTool);

    // ─── cdp_cookies(action, ...) — dispatches get/clear ───
    api.registerTool((_ctx) => defineCdpTool({
      name: "cdp_cookies",
      label: "读 / 清浏览器 Cookie",
      description: "action=get：按 URL 列表读当前授权测试 browser profile 的 cookie（含 HttpOnly）。action=clear：按 domain/name 清 cookie,用于测试账号状态切换和清理污染。",
      details: "合并 cdp_cookies_get + cdp_cookies_clear。get：传入 urls[] 数组，按 origin 匹配当前授权测试 profile 的 cookie jar 返回完整 cookie（含 HttpOnly，document.cookie 看不到的也能拿）。clear：三种模式 — 不传任何 filter→clearAll；传 domain→清该 domain 所有 cookie（含子域）；传 domain+name→精确清。触发场景：测试账号切换、已授权跨租户对比、清理污染状态。禁止用于真实用户或未授权账号。注意：需 profile 有活跃 CDP 客户端；name 模式必须同时传 domain 或 url。",
      schema: CdpCookiesSchema,
      async execute(params) {
        // 预检查 — 没活跃 CDP client 直接给友好 hint，对齐 cdp_query 风格
        const ps = _capture.getProfile(params.profile);
        const connected = (ps?.clients.size ?? 0) > 0;
        if (!connected) {
          return {
            cookies: params.action === "get" ? [] : undefined,
            cleared: params.action === "clear" ? 0 : undefined,
            hint: `profile ${params.profile} 没活跃 CDP 客户端 — 先用 browser 工具指定该 profile 打开页面再来读 cookie。`,
            profileConnected: false,
            profile: params.profile,
          };
        }
        if (params.action === "get") {
          return _capture.getCookies(params.profile, params.urls ?? []);
        } else {
          if (!params.domain && !params.name && !params.url) {
            return { error: "clear 模式需要至少一个 filter：domain / name+domain / name+url" };
          }
          return _capture.clearCookies(
            params.profile,
            { domain: params.domain, name: params.name, url: params.url },
          );
        }
      },
    }) as AnyAgentTool);
    ...
    api.registerTool((_ctx) => defineCdpTool({
      ...
      label: "Fetch 拦截控制",
      description: "控制 browser profile 的 Fetch domain 拦截。action: start（标记 URL pattern 开始拦截）/ list（列当前暂停的请求）/ continue（放行，可选 header_overrides 改包）/ fail（模拟网络失败）。补 Burp Community 缺的 Repeater 改包能力。",
      details: "合并 4 个 cdp_fetch_intercept_* 为单一工具。start：传入 url_pattern（substring 匹配完整 URL，空串匹配所有），匹配的请求不再自动 continue 而是暂停（30s 超时自动放行）。list：无额外参数，返回所有暂停请求 + 剩余超时。continue：传入 captured_request_id + 可选 header_overrides ({name:value} 字典) 修改请求头后放行。fail：传入 captured_request_id + 可选 error_reason 模拟网络失败。触发场景：cookie/fingerprint 研究 — 拦截并修改浏览器实际发出的 headers；CORS 测试 — 修改 Origin/Referer；前端异常处理研究 — 拦截并 fail 跟踪脚本。",
      ...
      async execute(params) {
        ...
        switch (params.action) {
          ...
          case "continue":
            return _capture.fetchContinue(params.profile, params.captured_request_id ?? "", params.header_overrides);
          case "fail":
    ...
    api.registerTool((_ctx) => defineCdpTool({
      name: "cdp_stats",
      label: "CDP 捕获统计",
      description: "返回所有 profile 的 CDP 捕获状态（连接 / 各 buffer 当前条数 / 最新时间戳）。先看这个判断 capture 是否工作。",
      details: "返回所有已配置 browser profile 的 CDP 捕获统计快照。每 profile 含：connected（CDP 客户端数）、各 buffer 当前条数。触发场景：诊断 capture 是否工作 — 看 traffic/console/exceptions 等 buffer 是否有数据；确认 profile 是否 connected。无额外参数，全量返回。",
      schema: CdpStatsSchema,
      async execute(params) {
        const all = _capture.stats();
        if (params.profile) return { [params.profile]: all[params.profile] ?? null };
        return all;
      },
    }) as AnyAgentTool);

    // ─── cdp_self_test(profile) — 保留 ───
    api.registerTool((_ctx) => defineCdpTool({
      name: "cdp_self_test",
      label: "CDP 健康自检",
      description: "L5 健康检查：检查指定 profile 的 CDP capture 是否真的工作。返回各 buffer 的状态 + 智能 recommendations（如：'frameEvents=0 但已连接' → 建议查 Page.enable 是否失败）。capture 行为奇怪时先调它。",
      details: "对指定 profile 做 L5 级健康检查。检查：各 buffer 状态、CDP 客户端连接数、当前 tracked origins。返回智能 recommendations（如：consoleLogs=0 但 connected → 可能 Runtime.enable 失败）。触发场景：capture 行为异常 — 明明 navigated 了页面但 buffer 没数据。注意：需 profile 有活跃 CDP 客户端。",
      schema: CdpSelfTestSchema,
      async execute(params) {
        return _capture.runSelfTest(params.profile);
      },
    }) as AnyAgentTool);
