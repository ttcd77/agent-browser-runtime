...
export const ProfileSchema = z
  .string()
  .min(1)
  .describe("浏览器 profile 名(lowercase)。命名约定 '<target>-<identity>' 或旧式 '<target>-<slot>'(如 'gocardless-merchant-a' / 'gocardless-payer' / '8x8-1'),每个独立 cookie/storage。配置在 ~/.openclaw/openclaw.json browser.profiles 段。需先用 browser 工具按该 profile 打开页面,再查询 CDP 捕获。");
...
// cdp_get: disc union over 2 single-entry fetchers
export const CdpGetType = z.enum(["traffic_detail", "script_source"]);
export const CdpGetSchema = z.object({
  profile: ProfileSchema,
  type: CdpGetType.describe("获取类型：traffic_detail（按 requestId）/ script_source（按 scriptId）"),
  id: z.string().min(1).describe("requestId（type=traffic_detail）或 scriptId（type=script_source）"),
});

// cdp_cookies: disc union over 2 actions
export const CdpCookiesAction = z.enum(["get", "clear"]);
export const CdpCookiesSchema = z.object({
  profile: ProfileSchema,
  action: CdpCookiesAction.describe("动作：get（读 cookie）/ clear（清 cookie）"),
  urls: z.array(z.string()).optional().describe("URL 列表，按 origin 匹配 cookie jar（action=get 时用）"),
  domain: z.string().optional().describe("清指定 domain 的 cookie（action=clear 时用）"),
  name: z.string().optional().describe("只清指定名字的 cookie（action=clear + domain 时用）"),
  url: z.string().optional().describe("配合 name 用，按 URL 自动找 domain/path"),
});
...
export const CdpFetchInterceptSchema = z.object({
  profile: ProfileSchema,
  action: CdpFetchAction.describe("动作：start（开始拦截）/ list（列暂停）/ continue（放行，可改 header）/ fail（模拟失败）"),
...
// cdp_stats — same schema, keep
export const CdpStatsSchema = z.object({
  profile: z.string().optional().describe("可选 profile 过滤；不填返回全量"),
});

// cdp_self_test — same schema, keep
export const CdpSelfTestSchema = z.object({
  profile: ProfileSchema,
});

// ────────── 核心：defineCdpTool helper ──────────

// 任意类型 — OpenClaw plugin SDK 用 typebox + 强制 cast，我们 runtime 给 raw object 即可
type AnyAgentTool = {
  name: string;
  label?: string;
  description: string;
