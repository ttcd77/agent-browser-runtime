...
try {
  ...
  await callTool(baseUrl, "browser_hard_reload", {
    profile: "default",
    waitMs: 800,
  });

  const security = await callTool(baseUrl, "browser_security_summary", { profile: "default" });
  assert(security.page?.url?.startsWith("https://example.com"), "security summary did not inspect example.com");
  const backendCapabilities = await callTool(baseUrl, "browser_backend_capabilities", { profile: "default" });
  ...
  const protocolSchema = await callTool(baseUrl, "browser_protocol_schema", { domain: "Network", query: "getResponseBody", limit: 5 });
  ...
  const browserCdp = await callTool(baseUrl, "browser_process_cdp", { method: "Browser.getVersion" });
  ...
  const diagnostics = await callTool(baseUrl, "browser_page_diagnostics", {
    profile: "default",
    limit: 5,
  });
  ...
  const networkSummary = await callTool(baseUrl, "profile_traffic_summary", {
    profile: "default",
    limit: 5,
  });
  assert(networkSummary.requestCount > 0, "network summary returned no requests");
  assert(networkSummary.recommendedDrilldowns?.some((entry) => entry.tool === "profile_request_detail"), `network summary missing request detail drilldown: ${JSON.stringify(networkSummary.recommendedDrilldowns)}`);
  const networkTimeline = await callTool(baseUrl, "profile_network_timeline", {
    profile: "default",
    limit: 5,
  });
  ...
  const requestDetail = await callTool(baseUrl, "profile_request_detail", {
    profile: "default",
  ...
  const issuesLog = await callTool(baseUrl, "browser_issues_log", {
    profile: "default",
    reload: false,
    waitMs: 100,
    limit: 5,
  ...
  const accessibility = await callTool(baseUrl, "browser_accessibility_snapshot", {
    profile: "default",
    maxNodes: 40,
  });
  assert(accessibility.nodeCount > 0, "accessibility snapshot returned no nodes");

  const domSnapshot = await callTool(baseUrl, "browser_dom_snapshot", {
    profile: "default",
  ...
  const domSearch = await callTool(baseUrl, "browser_dom_search", {
    profile: "default",
  ...
  const trace = await callTool(baseUrl, "browser_chrome_trace", {
    profile: "default",
    durationMs: 500,
    maxEvents: 5,
    maxScreenshots: 3,
  ...
  const traceQuery = await callTool(baseUrl, "browser_trace_query", {
    profile: "default",
  ...
  assert(traceQuery.recommendedDrilldowns?.some((entry) => entry.tool === "browser_chrome_trace"), `trace query missing fresh trace drilldown: ${JSON.stringify(traceQuery.recommendedDrilldowns)}`);
  if (traceQuery.events.length) {
    assert(traceQuery.recommendedDrilldowns?.some((entry) => entry.tool === "browser_trace_query" && entry.input?.tracePath === trace.tracePath), `trace query missing trace-query drilldown: ${JSON.stringify(traceQuery.recommendedDrilldowns)}`);
  }
  ...
  const secondTrace = await callTool(baseUrl, "browser_chrome_trace", {
    profile: "default",
    durationMs: 250,
    maxEvents: 5,
    maxScreenshots: 1,
  });
  const traceCompare = await callTool(baseUrl, "browser_trace_compare", {
    profile: "default",
  ...
  const performanceInsights = await callTool(baseUrl, "browser_performance_insights", {
    profile: "default",
    durationMs: 250,
  ...
  const performanceObserver = await callTool(baseUrl, "browser_performance_observer", {
    profile: "default",
    durationMs: 250,
    maxItems: 5,
    maxEntries: 50,
  ...
  const cpuProfile = await callTool(baseUrl, "browser_cpu_profile", {
    profile: "default",
    durationMs: 300,
    maxNodes: 10,
  ...
  const coverage = await callTool(baseUrl, "browser_coverage_snapshot", {
    profile: "default",
    durationMs: 500,
    maxEntries: 5,
  });
  ...
  assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_get" && entry.input?.scriptId), `source search missing source_get drilldown: ${JSON.stringify(sourceSearch.recommendedDrilldowns)}`);
  assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_pretty_print" && entry.input?.scriptId), `source search missing pretty-print drilldown: ${JSON.stringify(sourceSearch.recommendedDrilldowns)}`);
  ...
  const frameSearch = await callTool(baseUrl, "browser_dom_search", {
    profile: "default",
  ...
  // Parse frameIndexes from the framePath string (e.g. "top > frame[0]" → [0]).
  // CDP-backed tools (browser_css_styles, browser_event_listeners) accept framePath
  // directly and parse it via frameIndexesFromOptions. Playwright-backed tools
  // (browser_click, browser_type) pass framePath to page.frameLocator() which
  // treats it as a CSS selector — "frame[0]" is not valid CSS. Pass frameIndexes
  // to Playwright tools so they use frameLocator("iframe").nth(N) chaining instead.
  const frameIndexes = [...framePath.matchAll(/frame\[(\d+)\]/g)].map((m) => Number(m[1]));
  const frameStyles = await callTool(baseUrl, "browser_css_styles", {
    profile: "default",
  ...
  const frameListeners = await callTool(baseUrl, "browser_event_listeners", {
    profile: "default",
    selector: "#frame-action",
    framePath,
  });
  ...
  const frameClick = await callTool(baseUrl, "browser_click", {
    profile: "default",
    selector: "#frame-action",
    frameIndexes,
    waitMs: 100,
  ...
  const frameType = await callTool(baseUrl, "browser_type", {
    profile: "default",
  ...
  const frameState = await callTool(baseUrl, "browser_eval", {
    profile: "default",
  ...
  const keyboardType = await callTool(baseUrl, "browser_type", {
    profile: "default",
  ...
  const keyboardState = await callTool(baseUrl, "browser_eval", {
    profile: "default",
  ...
  const textClick = await callTool(baseUrl, "browser_click", {
    profile: "default",
    text: "Join meeting",
    waitMode: "no-navigation",
  });
  assert(textClick.ok === true && textClick.mode === "text", `text click failed: ${JSON.stringify(textClick)}`);
  assert(textClick.waitMode === "no-navigation" && textClick.waitMs === 120, `text click did not use SPA/no-navigation wait plan: ${JSON.stringify(textClick)}`);
  assert(textClick.afterObservation?.controls?.length >= 1, `text click missing afterObservation controls: ${JSON.stringify(textClick)}`);
  const textClickState = await callTool(baseUrl, "browser_eval", {
    profile: "default",
  ...
  const prettySource = await callTool(baseUrl, "browser_source_pretty_print", {
    profile: "default",
    query: sourceMarker,
    waitMs: 500,
    maxChars: 20000,
  ...
  const sourceMapMetadata = await callTool(baseUrl, "browser_source_map_metadata", {
    profile: "default",
    query: sourceMarker,
    waitMs: 500,
  });
  ...
  const sourceMapSources = await callTool(baseUrl, "browser_source_map_sources", {
    profile: "default",
    query: sourceMarker,
    waitMs: 500,
    maxSources: 5,
  ...
  const sourceMapSourceGet = await callTool(baseUrl, "browser_source_map_source_get", {
    profile: "default",
  ...
  const consoleLog = await callTool(baseUrl, "browser_console_log", {
    profile: "default",
    reload: true,
    waitMs: 800,
    limit: 20,
  ...
  const consoleSourceContext = await callTool(baseUrl, "browser_console_source_context", {
    profile: "default",
  ...
  const eventListeners = await callTool(baseUrl, "browser_event_listeners", {
    profile: "default",
  ...
  const cssStyles = await callTool(baseUrl, "browser_css_styles", {
    profile: "default",
    selector: "#agent-listener-button",
    maxRules: 20,
  });
  ...
  const hoverStyles = await callTool(baseUrl, "browser_css_styles", {
    profile: "default",
    selector: "#agent-listener-button",
  ...
  const mutationWatch = await callTool(baseUrl, "browser_dom_mutation_watch", {
    profile: "default",
    selector: "#agent-listener-button",
    durationMs: 300,
    maxEvents: 10,
  ...
  const rawCdp = await callTool(baseUrl, "browser_cdp_command", {
    profile: "default",
  ...
  const debuggerPause = await callTool(baseUrl, "browser_debugger_control", {
    profile: "default",
  ...
  await callTool(baseUrl, "browser_cdp_command", {
    profile: "default",
    method: "Runtime.evaluate",
    params: {
  ...
  const breakpointProbe = await callTool(baseUrl, "browser_debugger_control", {
    profile: "default",
  ...
  const tokenFlow = await callTool(baseUrl, "browser_token_flow_trace", {
    profile: "default",
    durationMs: 600,
    maxEvents: 20,
    triggerExpression: `
  ...
  const memorySnapshot = await callTool(baseUrl, "browser_memory_snapshot", {
    profile: "default",
  });
  ...
  const heapSnapshot = await callTool(baseUrl, "browser_heap_snapshot", {
    profile: "default",
  ...
  const coverageDetail = await callTool(baseUrl, "browser_coverage_detail", {
    profile: "default",
    durationMs: 800,
    maxEntries: 10,
  ...
  const globalSearch = await callTool(baseUrl, "browser_global_search", {
    profile: "default",
    query: sourceMarker,
    waitMs: 500,
    maxMatches: 30,
  ...
  const evidenceBundle = await callTool(baseUrl, "browser_evidence_bundle", {
    profile: "default",
    sourceLimit: 10,
    networkLimit: 5,
    includeHar: true,
  ...
  const researchPack = await callTool(baseUrl, "browser_security_research_pack", {
    profile: "default",
    limit: 5,
    waitMs: 500,
    includeTrace: true,
  ...
  assert(researchPack.handoffDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === researchPack.summary.researchPackPath), "security research pack missing handoff inspect drilldown");
  ...
  assert(researchPack.drilldownPlan?.drilldowns?.some((entry) => entry.tool === "browser_evidence_timeline"), "security research pack missing evidence timeline drilldown");
  ...
  assert(researchPack.nextTools?.includes("browser_artifact_index"), "security research pack nextTools missing artifact index");
  ...
  assert(toolCatalog.tools.some((tool) => tool.name === "browser_auth_boundary_report"), "tool catalog missing auth boundary report");
  ...
  const toolHelp = await callTool(baseUrl, "browser_tool_help", {
    profile: "default",
    tool: "browser_security_research_pack",
  });
  ...
  assert(capabilityMap.panels?.some((panel) => panel.category === "network" && panel.firstPass.includes("profile_traffic_summary")), "capability map missing Network first-pass tools");
  ...
  assert(capabilityMap.agentUsage?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail" && step.input?.requestId), "capability map missing request-detail panel route");
  assert(capabilityMap.agentUsage?.panelRoutes?.evidence?.some((step) => step.tool === "browser_artifact_inspect" && step.input?.path), "capability map missing artifact inspect panel route");
  const parityMatrix = await callTool(baseUrl, "browser_f12_parity_matrix", {
    profile: "default",
  });
  ...
  assert(workflowGuide.steps?.some((step) => step.tool === "browser_auth_boundary_report"), "workflow guide missing auth boundary step");
  ...
  const serviceWorker = await callTool(baseUrl, "browser_service_worker_summary", {
    profile: "default",
  });
  ...
  const serviceWorkerDetail = await callTool(baseUrl, "browser_service_worker_detail", {
    profile: "default",
  ...
  const workerFrameDeepDive = await callTool(baseUrl, "browser_worker_frame_deep_dive", {
    profile: "default",
    save: true,
  });
  ...
  await callTool(baseUrl, "browser_eval", {
    profile: "default",
    expression: `
  ...
  const realtimeLog = await callTool(baseUrl, "profile_realtime_log", {
    profile: "default",
    limit: 20,
  });
  ...
  const realtimePayloadFilter = await callTool(baseUrl, "profile_realtime_log", {
    profile: "default",
  ...
  assert(realtimePayloadFilter.recommendedDrilldowns?.some((entry) => entry.tool === "profile_realtime_log" && entry.input?.requestId), `realtime payload filter missing concrete drilldown: ${JSON.stringify(realtimePayloadFilter.recommendedDrilldowns)}`);
  ...
  const realtimeSseFilter = await callTool(baseUrl, "profile_realtime_log", {
    profile: "default",
  ...
  const redirectSummary = await callTool(baseUrl, "profile_traffic_summary", {
    profile: "default",
    limit: 50,
  });
  ...
  const redirectDetail = await callTool(baseUrl, "profile_request_detail", {
    profile: "default",
  ...
  const filteredRedirects = await callTool(baseUrl, "profile_traffic_query", {
    profile: "default",
    redirected: true,
    status_min: 200,
    status_max: 299,
  ...
  const filteredTimeline = await callTool(baseUrl, "profile_network_timeline", {
    profile: "default",
  ...
  const requestGraph = await callTool(baseUrl, "browser_request_correlation_graph", {
    profile: "default",
    limit: 200,
  });
  ...
  const replayTraffic = await callTool(baseUrl, "profile_traffic_query", {
    profile: "default",
    limit: 20,
  });
  ...
  const echoDetail = await callTool(baseUrl, "profile_request_detail", {
    profile: "default",
  ...
  const initiatorGraph = await callTool(baseUrl, "browser_request_correlation_graph", {
    profile: "default",
    limit: 200,
  });
  ...
  const formReplay = await callTool(baseUrl, "profile_request_replay", {
    profile: "default",
    requestId: echoRequest.requestId,
    headers: {
  ...
  const multipartReplay = await callTool(baseUrl, "profile_request_replay", {
    profile: "default",
    requestId: echoRequest.requestId,
    headers: {
  ...
  const batchReplay = await callTool(baseUrl, "profile_request_replay_batch", {
    profile: "default",
    requestId: echoRequest.requestId,
    variants: [
  ...
  const applicationExport = await callTool(baseUrl, "browser_application_export", {
    profile: "default",
  ...
  const indexedDbList = await callTool(baseUrl, "browser_indexeddb_list", {
    profile: "default",
    maxDatabases: 20,
  });
  ...
  const indexedDbRead = await callTool(baseUrl, "browser_indexeddb_read", {
    profile: "default",
  ...
  const cacheStorageList = await callTool(baseUrl, "browser_cache_storage_list", {
    profile: "default",
    maxCaches: 20,
    maxEntries: 20,
  });
  ...
  const cacheEntry = await callTool(baseUrl, "browser_cache_entry_get", {
    profile: "default",
  ...
  const applicationSearch = await callTool(baseUrl, "browser_global_search", {
    profile: "default",
  ...
  const cookieSummary = await callTool(baseUrl, "browser_cookie_summary", {
    profile: "default",
  });
  ...
  const storageOrigin = await callTool(baseUrl, "browser_storage_origin_summary", {
    profile: "default",
  });
  ...
  const chipsAttempt = await callTool(baseUrl, "browser_eval", {
    profile: "default",
  ...
  const signalSummary = await callTool(baseUrl, "browser_signal_summary", {
    profile: "default",
  });
  ...
  assert(facadeRaw.result?.page?.url, "browser_raw did not return wrapped browser_page_diagnostics evidence");
  ...
  const captureBisect = await callTool(baseUrl, "browser_capture_bisect", {
    profile: "default",
    limit: 20,
  });
  ...
  const savedHar = await callTool(baseUrl, "profile_save_har", {
    profile: "default",
    limit: 20,
  });
  assert(savedHar.harPath, "saved HAR path missing");
  assert(savedHar.harBytes > 0, "saved HAR was empty");
  const harArtifact = await callTool(baseUrl, "browser_artifact_inspect", {
  ...
  assert(artifactIndex.recommendedDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path), `artifact index missing recommended inspect drilldown: ${JSON.stringify(artifactIndex.recommendedDrilldowns)}`);
  const handoffArtifactIndex = await callTool(baseUrl, "browser_artifact_index", {
    profile: "default",
    maxFiles: 200,
  });
  ...
  assert(handoffArtifactIndex.recommendedDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === handoffArtifactIndex.latestByKind?.["research-pack"]?.path), `artifact index missing latest research-pack drilldown: ${JSON.stringify(handoffArtifactIndex.recommendedDrilldowns)}`);
  const artifactSearch = await callTool(baseUrl, "browser_artifact_search", {
    profile: "default",
    query: "Agent Browser Runtime",
    kind: "har",
    maxFiles: 20,
  ...
  const artifactRead = await callTool(baseUrl, "browser_artifact_read", {
    path: savedHar.harPath,
    startLine: 1,
    lineCount: 12,
  });
  ...
  const evidenceTimeline = await callTool(baseUrl, "browser_evidence_timeline", {
    profile: "default",
    maxEvents: 100,
    maxArtifacts: 30,
  });
  ...
  const artifactTimeline = await callTool(baseUrl, "browser_evidence_timeline", {
    profile: "default",
    eventType: "artifact",
    query: "har",
    maxEvents: 20,
  ...
  const harWithBodies = await callTool(baseUrl, "profile_export_har", {
    profile: "default",
    limit: 20,
    includeBodies: true,
    maxBodyBytes: 2000,
  ...
  const harCompleteness = await callTool(baseUrl, "profile_har_completeness", {
    profile: "default",
    limit: 20,
    includeBodies: true,
    maxBodyBytes: 2000,
  ...
  if (harCompleteness.drilldownSamples.bodyMissing.length || harCompleteness.drilldownSamples.timingMissing.length || harCompleteness.drilldownSamples.redirects.length || harCompleteness.drilldownSamples.securityMissing.length) {
    assert(harCompleteness.recommendedDrilldowns.some((entry) => entry.tool === "profile_request_detail"), `HAR completeness missing request detail recommendation: ${JSON.stringify(harCompleteness.recommendedDrilldowns)}`);
  }
