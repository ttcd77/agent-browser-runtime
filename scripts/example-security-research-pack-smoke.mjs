...
try {
  ...
  assert(output.afterReadiness?.routeSummary?.latestHandoffInspect?.tool === "browser_artifact_inspect", `example missing routeSummary handoff inspect: ${JSON.stringify(output.afterReadiness?.routeSummary)}`);
  assert(output.afterReadiness?.routeSummary?.latestHandoffRead?.tool === "browser_artifact_read", `example missing routeSummary handoff read: ${JSON.stringify(output.afterReadiness?.routeSummary)}`);
  assert(output.afterReadiness?.routeSummary?.firstF12RequestDetail?.tool === "profile_request_detail", `example missing routeSummary F12 request detail: ${JSON.stringify(output.afterReadiness?.routeSummary)}`);
  assert(output.afterReadiness?.routeSummary?.firstConcreteDrilldown?.tool === "profile_request_detail", `example missing routeSummary concrete drilldown: ${JSON.stringify(output.afterReadiness?.routeSummary)}`);
  assert(output.afterReadiness?.routeArtifacts?.f12Navigation?.inspect?.tool === "browser_artifact_inspect", `example missing compact F12 navigation route artifact: ${JSON.stringify(output.afterReadiness?.routeArtifacts)}`);
  assert(output.afterReadiness?.routeArtifacts?.harCompleteness?.read?.tool === "browser_artifact_read", `example missing compact HAR completeness route artifact: ${JSON.stringify(output.afterReadiness?.routeArtifacts)}`);
  assert(output.afterReadiness?.routeArtifacts?.correlationGraph?.inspect?.tool === "browser_artifact_inspect", `example missing compact correlation graph route artifact: ${JSON.stringify(output.afterReadiness?.routeArtifacts)}`);
  assert(output.afterReadiness?.routeArtifacts?.authBoundary?.read?.tool === "browser_artifact_read", `example missing compact auth boundary route artifact: ${JSON.stringify(output.afterReadiness?.routeArtifacts)}`);
  assert(output.afterReadiness?.routeArtifacts?.workerFrameBoundary?.inspect?.tool === "browser_artifact_inspect", `example missing compact worker/frame route artifact: ${JSON.stringify(output.afterReadiness?.routeArtifacts)}`);
  ...
  assert(output.f12Navigation?.firstDetailRoute?.tool === "profile_request_detail", `example missing pack F12 detail route: ${JSON.stringify(output.f12Navigation)}`);
  ...
  assert(
    output.operatorHandoff.firstRequest?.tool === "profile_request_detail",
    `operatorHandoff firstRequest must be profile_request_detail: ${JSON.stringify(output.operatorHandoff?.firstRequest)}`,
  );
  assert(
  ...
  assert(
    demoReportMd.includes("browser_artifact_read"),
    "demo report missing browser_artifact_read tool reference",
  );
