...
const capabilities = await callTool("browser_backend_capabilities");
...
const protocolSchema = await callTool("browser_protocol_schema", { domain: "Network", query: "getResponseBody" });
...
const browserCdp = await callTool("browser_process_cdp", { method: "Browser.getVersion" });
...
const browserVersion = await callTool("browser_process_version");
...
const browserTargets = await callTool("browser_process_targets");
assert(browserTargets.targetCount >= 1, `Personal Chrome browser targets missing tabs: ${JSON.stringify(browserTargets)}`);
const systemInfo = await callTool("browser_system_info");
...
const status = await callTool("browser_profile_status");
...
await callTool("browser_capture_start", {
  clear: true,
...
const sourceSearch = await callTool("browser_sources_search", {
...
assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_get" && entry.input?.scriptId), `Personal Chrome source search missing source_get drilldown: ${JSON.stringify(sourceSearch.recommendedDrilldowns)}`);
assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_pretty_print" && entry.input?.scriptId), `Personal Chrome source search missing pretty-print drilldown: ${JSON.stringify(sourceSearch.recommendedDrilldowns)}`);
...
const redirectSummary = await callTool("profile_traffic_summary", {
  limit: 50,
});
...
const redirectDetail = await callTool("profile_request_detail", {
...
const requestGraph = await callTool("browser_request_correlation_graph", {
  limit: 200,
});
...
const filteredRedirects = await callTool("profile_traffic_query", {
  redirected: true,
  status_min: 200,
  status_max: 299,
  resource_type: "Fetch",
...
const filteredTimeline = await callTool("profile_network_timeline", {
...
const replay = await callTool("profile_request_replay", {
  requestId: redirectRow.requestId,
  headers: {
...
const frameTree = await callTool("browser_frame_tree");
...
const storage = await callTool("browser_storage_origin_summary");
...
const indexedDbList = await callTool("browser_indexeddb_list", {
  maxDatabases: 20,
});
...
const indexedDbRead = await callTool("browser_indexeddb_read", {
...
for (let attempt = 0; attempt < 12; attempt += 1) {
  cacheStorageList = await callTool("browser_cache_storage_list", {
    maxCaches: 20,
    maxEntries: 50,
  });
...
const cacheEntry = await callTool("browser_cache_entry_get", {
...
const applicationSearch = await callTool("browser_global_search", {
...
const performanceInsights = await callTool("browser_performance_insights", {
  durationMs: 250,
...
const performanceObserver = await callTool("browser_performance_observer", {
  durationMs: 250,
  maxItems: 5,
  maxEntries: 50,
});
...
const realtimeLog = await callTool("profile_realtime_log", {
  limit: 5,
});
...
const realtimeFiltered = await callTool("profile_realtime_log", {
...
const heapSnapshot = await callTool("browser_heap_snapshot");
...
assert(heapSnapshot.managedFallbackTool === "browser_heap_snapshot", "Personal Chrome heap snapshot missing managed fallback guidance");

const chromeTrace = await callTool("browser_chrome_trace", {
  durationMs: 250,
  maxEvents: 5,
...
const traceQuery = await callTool("browser_trace_query", {
...
assert(traceQuery.recommendedDrilldowns?.some((entry) => entry.tool === "browser_chrome_trace"), `Personal Chrome trace query missing fresh trace drilldown: ${JSON.stringify(traceQuery.recommendedDrilldowns)}`);
if (traceQuery.events.length) {
  assert(traceQuery.recommendedDrilldowns?.some((entry) => entry.tool === "browser_trace_query" && entry.input?.tracePath === chromeTrace.tracePath), `Personal Chrome trace query missing trace-query drilldown: ${JSON.stringify(traceQuery.recommendedDrilldowns)}`);
}
...
const secondChromeTrace = await callTool("browser_chrome_trace", {
  durationMs: 250,
  maxEvents: 5,
  saveScreenshots: false,
});
const traceCompare = await callTool("browser_trace_compare", {
...
const researchPack = await callTool("browser_security_research_pack", {
  limit: 5,
  waitMs: 500,
  includeTrace: true,
  includeHar: true,
...
assert(researchPack.summary?.tracePath, "Personal Chrome security research pack missing trace path");
...
assert(researchPack.f12Navigation?.requests?.some((row) => row.f12Columns?.name && row.detail?.tool === "profile_request_detail"), "Personal Chrome security research pack missing F12 navigation request detail route");
...
assert(researchPack.summary?.firstF12RequestDetailPath, "Personal Chrome security research pack missing standalone first F12 request detail path");
assert(researchPack.artifacts?.firstF12RequestDetail?.sha256, "Personal Chrome security research pack missing standalone first F12 request detail artifact hash");
...
assert(researchPack.agentUsage?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "Personal Chrome security research pack missing network panel usage route");
...
assert(researchPack.handoffDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === researchPack.summary.researchPackPath), "Personal Chrome security research pack missing handoff inspect drilldown");
...
assert(researchPack.drilldownPlan?.drilldowns?.some((entry) => entry.tool === "browser_evidence_timeline"), "Personal Chrome security research pack missing evidence timeline drilldown");
...
const researchPackInspect = await callTool("browser_artifact_inspect", {
...
assert(researchPackInspect.researchPackHandoff?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "Personal Chrome handoff inspect missing network panel route");
...
assert(facadeInspect.result?.professionalWorkflow?.routeSummaryTemplate?.firstStep?.tool === "browser_professional_readiness", "Personal Chrome browser_inspect missing route template first step");
...
assert(facadePack.summary?.tracePath, "Personal Chrome browser_security_pack missing trace path");
assert(facadePack.parityMatrix?.backend === "personal-chrome", "Personal Chrome browser_security_pack missing parity snapshot");

const captureBisect = await callTool("browser_capture_bisect", {
  limit: 20,
  save: false,
});
...
const harCompleteness = await callTool("profile_har_completeness", {
  limit: 20,
  includeBodies: false,
  save: false,
});
...
if (
  ...
  assert(harCompleteness.recommendedDrilldowns.some((entry) => entry.tool === "profile_request_detail"), `Personal Chrome HAR completeness missing request detail recommendation: ${JSON.stringify(harCompleteness.recommendedDrilldowns)}`);
}
...
const harWithBodies = await callTool("profile_export_har", {
  limit: 20,
  includeBodies: true,
  maxBodyBytes: 2000,
});
...
const savedHar = await callTool("profile_save_har", {
  limit: 20,
  includeBodies: false,
});
assert(savedHar.harPath, "Personal Chrome saved HAR path missing");
const harArtifact = await callTool("browser_artifact_inspect", {
...
const artifactIndex = await callTool("browser_artifact_index", {
  kind: "har",
  maxFiles: 20,
});
...
assert(artifactIndex.recommendedDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path), `Personal Chrome artifact index missing recommended inspect drilldown: ${JSON.stringify(artifactIndex.recommendedDrilldowns)}`);
const handoffArtifactIndex = await callTool("browser_artifact_index", {
  maxFiles: 200,
});
...
assert(handoffArtifactIndex.recommendedDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === handoffArtifactIndex.latestByKind?.["research-pack"]?.path), `Personal Chrome artifact index missing latest research-pack drilldown: ${JSON.stringify(handoffArtifactIndex.recommendedDrilldowns)}`);
const artifactSearch = await callTool("browser_artifact_search", {
  query: "Agent Browser Runtime",
  kind: "har",
  maxFiles: 20,
});
...
const artifactRead = await callTool("browser_artifact_read", {
  path: savedHar.harPath,
  startLine: 1,
  lineCount: 12,
});
...
const evidenceTimeline = await callTool("browser_evidence_timeline", {
  maxEvents: 100,
  maxArtifacts: 30,
});
...
const artifactTimeline = await callTool("browser_evidence_timeline", {
  eventType: "artifact",
  query: "har",
  maxEvents: 20,
  maxArtifacts: 50,
...
const toolCatalog = await callTool("browser_tool_catalog", { query: "auth" });
assert(toolCatalog.toolCount >= 1, "Personal Chrome tool catalog did not return auth tools");
assert(toolCatalog.tools.some((tool) => tool.name === "browser_auth_boundary_report"), "Personal Chrome tool catalog missing auth boundary report");
...
assert(toolCatalog.agentEntryPoints?.professionalRouteSummary?.firstStep?.tool === "browser_professional_readiness", "Personal Chrome tool catalog missing route summary first step");
...
assert(toolCatalog.agentEntryPoints?.professionalRouteSummary?.handoffInspectTemplate?.tool === "browser_artifact_inspect", "Personal Chrome tool catalog missing route summary handoff inspect template");
...
const toolHelp = await callTool("browser_tool_help", { tool: "browser_security_research_pack" });
assert(toolHelp.description, "Personal Chrome tool help missing description");
const capabilityMap = await callTool("browser_capability_map", {});
...
assert(capabilityMap.agentUsage?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail" && step.input?.requestId), "Personal Chrome capability map missing request-detail panel route");
assert(capabilityMap.agentUsage?.panelRoutes?.evidence?.some((step) => step.tool === "browser_artifact_inspect" && step.input?.path), "Personal Chrome capability map missing artifact inspect panel route");
const parityMatrix = await callTool("browser_f12_parity_matrix", {});
...
const professionalReadiness = await callTool("browser_professional_readiness", {});
...
assert(professionalReadiness.latestResearchPackHandoff?.inspect?.tool === "browser_artifact_inspect", "Personal Chrome professional readiness missing latest handoff inspect route");
...
assert(professionalReadiness.checks?.some((check) => check.name === "f12NavigationReachable" && check.present), `Personal Chrome professional readiness missing F12 navigation check: ${JSON.stringify(professionalReadiness.checks)}`);
assert(professionalReadiness.f12Navigation?.requestNodeCount >= 1, `Personal Chrome professional readiness missing F12 navigation: ${JSON.stringify(professionalReadiness.f12Navigation)}`);
assert(professionalReadiness.f12NavigationDrilldowns?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), `Personal Chrome professional readiness missing F12 navigation drilldown: ${JSON.stringify(professionalReadiness.f12NavigationDrilldowns)}`);
assert(professionalReadiness.researchPackDrilldowns?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), `Personal Chrome professional readiness missing research-pack request drilldown: ${JSON.stringify(professionalReadiness.researchPackDrilldowns)}`);
...
assert(professionalReadiness.latestArtifacts?.har?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing latest HAR artifact pointer: ${JSON.stringify(professionalReadiness.latestArtifacts)}`);
assert(professionalReadiness.latestArtifacts?.["research-pack"]?.read?.tool === "browser_artifact_read", `Personal Chrome professional readiness missing latest research-pack artifact pointer: ${JSON.stringify(professionalReadiness.latestArtifacts)}`);
...
assert(professionalReadiness.evidenceEntrypoints?.correlationGraph?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing correlation graph entrypoint: ${JSON.stringify(professionalReadiness.evidenceEntrypoints)}`);
assert(professionalReadiness.evidenceEntrypoints?.authBoundary?.read?.tool === "browser_artifact_read", `Personal Chrome professional readiness missing auth boundary entrypoint: ${JSON.stringify(professionalReadiness.evidenceEntrypoints)}`);
assert(professionalReadiness.evidenceEntrypoints?.workerFrameBoundary?.read?.tool === "browser_artifact_read", `Personal Chrome professional readiness missing worker/frame boundary entrypoint: ${JSON.stringify(professionalReadiness.evidenceEntrypoints)}`);
...
assert(professionalReadiness.routeSummary?.latestHandoffInspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing route handoff inspect: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeArtifacts?.f12Navigation?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing compact F12 navigation route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.harCompleteness?.read?.tool === "browser_artifact_read", `Personal Chrome professional readiness missing compact HAR completeness route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.correlationGraph?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing compact correlation graph route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.authBoundary?.read?.tool === "browser_artifact_read", `Personal Chrome professional readiness missing compact auth boundary route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.workerFrameBoundary?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing compact worker/frame route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeSummary?.harCompletenessArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing HAR completeness artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.harCompletenessArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.harCompletenessPath, `Personal Chrome professional readiness HAR completeness artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.traceArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing trace artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.traceArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.tracePath, `Personal Chrome professional readiness trace artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.traceQuery?.tool === "browser_trace_query", `Personal Chrome professional readiness missing trace query route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.applicationExportArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing Application export artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.applicationExportArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.applicationExportPath, `Personal Chrome professional readiness Application export artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.evidenceBundleArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing evidence bundle artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.evidenceBundleArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.evidenceBundlePath, `Personal Chrome professional readiness evidence bundle artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.drilldownPlanArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing drilldown plan artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.drilldownPlanArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.drilldownPlanPath, `Personal Chrome professional readiness drilldown plan artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.evidenceManifestArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing evidence manifest artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.evidenceManifestArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.evidenceManifestPath, `Personal Chrome professional readiness evidence manifest artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.correlationGraphArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing correlation graph artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.correlationGraphArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.correlationGraphPath, `Personal Chrome professional readiness correlation graph artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.authBoundaryArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing auth boundary artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.authBoundaryArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.authBoundaryReportPath, `Personal Chrome professional readiness auth boundary artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.workerFrameArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing worker/frame artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.workerFrameArtifact?.path === professionalReadiness.latestResearchPackSummary?.artifactPaths?.workerFrameReportPath, `Personal Chrome professional readiness worker/frame artifact path mismatch: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.f12NavigationArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing F12 navigation artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.firstF12RequestDetailArtifact?.inspect?.tool === "browser_artifact_inspect", `Personal Chrome professional readiness missing first F12 request-detail artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.firstF12RequestDetail?.tool === "profile_request_detail", `Personal Chrome professional readiness missing F12 navigation request-detail route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.firstF12RequestDetail?.f12Columns?.name, `Personal Chrome professional readiness missing F12 navigation columns: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.firstConcreteDrilldown?.tool === "profile_request_detail", `Personal Chrome professional readiness missing route concrete request drilldown: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "Personal Chrome professional readiness missing network panel route");
assert(professionalReadiness.artifactDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path), "Personal Chrome professional readiness missing artifact drilldown route");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.harCompletenessArtifact?.path), "Personal Chrome professional readiness missing HAR completeness artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_trace_query" && entry.input?.tracePath === professionalReadiness.routeSummary?.traceArtifact?.path), "Personal Chrome professional readiness missing trace query next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.applicationExportArtifact?.path), "Personal Chrome professional readiness missing Application export artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.evidenceBundleArtifact?.path), "Personal Chrome professional readiness missing evidence bundle artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.drilldownPlanArtifact?.path), "Personal Chrome professional readiness missing drilldown plan artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.evidenceManifestArtifact?.path), "Personal Chrome professional readiness missing evidence manifest artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.correlationGraphArtifact?.path), "Personal Chrome professional readiness missing correlation graph artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.authBoundaryArtifact?.path), "Personal Chrome professional readiness missing auth boundary artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.workerFrameArtifact?.path), "Personal Chrome professional readiness missing worker/frame artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.f12NavigationArtifact?.path), "Personal Chrome professional readiness missing F12 navigation artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.firstF12RequestDetailArtifact?.path), "Personal Chrome professional readiness missing first F12 request-detail artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), "Personal Chrome professional readiness missing research-pack request-detail next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_read" && entry.input?.path), "Personal Chrome professional readiness missing artifact-read next action");
...
const workflowGuide = await callTool("browser_workflow_guide", { task: "auth-boundary" });
assert(workflowGuide.steps?.some((step) => step.tool === "browser_auth_boundary_report"), "Personal Chrome workflow guide missing auth boundary step");
const professionalWorkflowGuide = await callTool("browser_workflow_guide", { task: "professional-appsec" });
assert(professionalWorkflowGuide.steps?.some((step) => step.tool === "browser_professional_readiness"), "Personal Chrome professional workflow guide missing readiness step");
assert(professionalWorkflowGuide.routeSummaryTemplate?.firstStep?.tool === "browser_professional_readiness", "Personal Chrome professional workflow guide missing route template first step");
...
assert(professionalWorkflowGuide.routeSummaryTemplate?.latestHandoffInspect?.tool === "browser_artifact_inspect", "Personal Chrome professional workflow guide missing route template handoff inspect");
