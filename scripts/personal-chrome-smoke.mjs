...
assert(professionalReadiness.f12NavigationDrilldowns?.some((entry) => entry.tool === "devtools_request_detail" && entry.input?.requestId), `Personal Chrome professional readiness missing F12 navigation drilldown: ${JSON.stringify(professionalReadiness.f12NavigationDrilldowns)}`);
...
assert(professionalReadiness.routeArtifacts?.f12Navigation?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing compact F12 navigation route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.harCompleteness?.read?.tool === "devtools_artifact_read", `Personal Chrome professional readiness missing compact HAR completeness route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.correlationGraph?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing compact correlation graph route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.authBoundary?.read?.tool === "devtools_artifact_read", `Personal Chrome professional readiness missing compact auth boundary route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeArtifacts?.workerFrameBoundary?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing compact worker/frame route artifact: ${JSON.stringify(professionalReadiness.routeArtifacts)}`);
assert(professionalReadiness.routeSummary?.harCompletenessArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing HAR completeness artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.traceArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing trace artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.traceQuery?.tool === "devtools_trace_query", `Personal Chrome professional readiness missing trace query route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
assert(professionalReadiness.routeSummary?.applicationExportArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing Application export artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.evidenceBundleArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing evidence bundle artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.drilldownPlanArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing drilldown plan artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.evidenceManifestArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing evidence manifest artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.correlationGraphArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing correlation graph artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.authBoundaryArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing auth boundary artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.routeSummary?.workerFrameArtifact?.inspect?.tool === "devtools_artifact_inspect", `Personal Chrome professional readiness missing worker/frame artifact inspect route: ${JSON.stringify(professionalReadiness.routeSummary)}`);
...
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.harCompletenessArtifact?.path), "Personal Chrome professional readiness missing HAR completeness artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_trace_query" && entry.input?.tracePath === professionalReadiness.routeSummary?.traceArtifact?.path), "Personal Chrome professional readiness missing trace query next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.applicationExportArtifact?.path), "Personal Chrome professional readiness missing Application export artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.evidenceBundleArtifact?.path), "Personal Chrome professional readiness missing evidence bundle artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.drilldownPlanArtifact?.path), "Personal Chrome professional readiness missing drilldown plan artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.evidenceManifestArtifact?.path), "Personal Chrome professional readiness missing evidence manifest artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.correlationGraphArtifact?.path), "Personal Chrome professional readiness missing correlation graph artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.authBoundaryArtifact?.path), "Personal Chrome professional readiness missing auth boundary artifact next action");
assert(professionalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === professionalReadiness.routeSummary?.workerFrameArtifact?.path), "Personal Chrome professional readiness missing worker/frame artifact next action");
