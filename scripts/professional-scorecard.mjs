...
try {
  ...
  const [parity, capabilityMap, workflow, readiness, toolCatalog] = await Promise.all([
    callTool(baseUrl, "browser_f12_parity_matrix", { profile }),
    callTool(baseUrl, "browser_capability_map", { profile }),
    callTool(baseUrl, "browser_workflow_guide", { profile, task: "professional-appsec" }),
    callTool(baseUrl, "browser_professional_readiness", { profile }),
    callTool(baseUrl, "browser_tool_catalog", { profile }),
  ]);
