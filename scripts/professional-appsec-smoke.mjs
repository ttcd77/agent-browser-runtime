...
try {
  ...
  const capabilityMap = await callTool(baseUrl, "browser_capability_map", {
    profile: "professional",
  });
  ...
  const toolCatalog = await callTool(baseUrl, "browser_tool_catalog", {
    profile: "professional",
    query: "auth",
  });
  assert(toolCatalog.agentEntryPoints?.professionalRouteSummary?.firstStep?.tool === "browser_professional_readiness", "professional tool catalog missing route summary first step");
  ...
  assert(toolCatalog.agentEntryPoints?.professionalRouteSummary?.handoffInspectTemplate?.tool === "browser_artifact_inspect", "professional tool catalog missing route summary handoff inspect template");
  ...
  const workflowGuide = await callTool(baseUrl, "browser_workflow_guide", {
    profile: "professional",
  ...
  assert(workflowGuide.steps?.some((step) => step.tool === "browser_professional_readiness"), "professional workflow guide missing readiness step");
  ...
  assert(workflowGuide.routeSummaryTemplate?.firstStep?.tool === "browser_professional_readiness", "professional workflow guide missing route template first step");
  ...
  assert(workflowGuide.routeSummaryTemplate?.latestHandoffInspect?.tool === "browser_artifact_inspect", "professional workflow guide missing route template handoff inspect");
  ...
  const initialReadiness = await callTool(baseUrl, "browser_professional_readiness", {
    profile: "professional",
  });
  ...
  assert(initialReadiness.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "professional readiness missing network panel route");
  ...
  assert(firstInspect.result?.professionalWorkflow?.readinessTool === "browser_professional_readiness", "professional browser_inspect missing readiness tool route");
  assert(firstInspect.result?.professionalWorkflow?.routeSummaryTemplate?.firstStep?.tool === "browser_professional_readiness", "professional browser_inspect missing route template first step");
  ...
  const pack = await callTool(baseUrl, "browser_security_research_pack", {
    profile: "professional",
    url: fixture.url,
    waitMs: 1200,
    includeHar: true,
  ...
  assert(pack.summary?.f12NavigationPath, "professional pack missing standalone F12 navigation path");
  assert(pack.artifacts?.f12Navigation?.sha256, "professional pack missing standalone F12 navigation artifact hash");
  assert(pack.f12Navigation?.requestNodeCount >= 1, `professional pack missing F12 navigation: ${JSON.stringify(pack.f12Navigation)}`);
  assert(pack.f12Navigation?.requests?.some((row) => row.f12Columns?.name && row.detail?.tool === "profile_request_detail"), "professional pack missing F12 navigation request detail route");
  ...
  assert(pack.agentUsage?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "professional pack missing network panel usage route");
  ...
  assert(pack.handoffDrilldowns?.some((entry) => entry.tool === "browser_artifact_read" && entry.input?.path === pack.summary.researchPackPath), "professional pack missing handoff read drilldown");
  ...
  const finalReadiness = await callTool(baseUrl, "browser_professional_readiness", {
    profile: "professional",
  });
  ...
  assert(finalReadiness.latestArtifacts?.har?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing latest HAR artifact pointer: ${JSON.stringify(finalReadiness.latestArtifacts)}`);
  assert(finalReadiness.latestArtifacts?.["research-pack"]?.read?.tool === "browser_artifact_read", `professional readiness missing latest research-pack artifact pointer: ${JSON.stringify(finalReadiness.latestArtifacts)}`);
  ...
  assert(finalReadiness.evidenceEntrypoints?.correlationGraph?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing correlation graph entrypoint: ${JSON.stringify(finalReadiness.evidenceEntrypoints)}`);
  assert(finalReadiness.evidenceEntrypoints?.authBoundary?.read?.tool === "browser_artifact_read", `professional readiness missing auth boundary entrypoint: ${JSON.stringify(finalReadiness.evidenceEntrypoints)}`);
  assert(finalReadiness.evidenceEntrypoints?.workerFrameBoundary?.read?.tool === "browser_artifact_read", `professional readiness missing worker/frame boundary entrypoint: ${JSON.stringify(finalReadiness.evidenceEntrypoints)}`);
  ...
  assert(finalReadiness.routeSummary?.latestHandoffInspect?.tool === "browser_artifact_inspect", `professional readiness missing route handoff inspect: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeArtifacts?.f12Navigation?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing compact F12 navigation route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeArtifacts?.realtimeLog?.read?.tool === "browser_artifact_read", `professional readiness missing compact realtime route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeArtifacts?.harCompleteness?.read?.tool === "browser_artifact_read", `professional readiness missing compact HAR completeness route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeArtifacts?.correlationGraph?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing compact correlation graph route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeArtifacts?.authBoundary?.read?.tool === "browser_artifact_read", `professional readiness missing compact auth boundary route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeArtifacts?.workerFrameBoundary?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing compact worker/frame route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  assert(finalReadiness.routeSummary?.harCompletenessArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing HAR completeness artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.traceArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing trace artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.traceQuery?.tool === "browser_trace_query", `professional readiness missing trace query route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.applicationExportArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing Application export artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.evidenceBundleArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing evidence bundle artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.evidenceBundleArtifact?.path === pack.summary.evidenceBundlePath, `professional readiness evidence bundle artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.drilldownPlanArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing drilldown plan artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.drilldownPlanArtifact?.path === pack.summary.drilldownPlanPath, `professional readiness drilldown plan artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.evidenceManifestArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing evidence manifest artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.evidenceManifestArtifact?.path === pack.summary.evidenceManifestPath, `professional readiness evidence manifest artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.correlationGraphArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing correlation graph artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.correlationGraphArtifact?.path === pack.summary.correlationGraphPath, `professional readiness correlation graph artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.authBoundaryArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing auth boundary artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.authBoundaryArtifact?.path === pack.summary.authBoundaryReportPath, `professional readiness auth boundary artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.workerFrameArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing worker/frame artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.workerFrameArtifact?.path === pack.summary.workerFrameReportPath, `professional readiness worker/frame artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.f12NavigationArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing F12 navigation artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.firstF12RequestDetailArtifact?.inspect?.tool === "browser_artifact_inspect", `professional readiness missing first F12 request-detail artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.firstF12RequestDetailArtifact?.path === pack.summary.firstF12RequestDetailPath, `professional readiness first F12 request-detail artifact path mismatch: ${JSON.stringify(finalReadiness.routeSummary)}`);
  assert(finalReadiness.routeSummary?.firstF12RequestDetail?.tool === "profile_request_detail", `professional readiness missing F12 navigation request-detail route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.firstConcreteDrilldown?.tool === "profile_request_detail", `professional readiness missing route concrete request drilldown: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.captureBuckets?.networkRequestCount >= 1, `professional readiness missing capture bucket summary: ${JSON.stringify(finalReadiness.captureBuckets)}`);
  assert(finalReadiness.checks?.some((check) => check.name === "captureBisectReachable" && check.present), `professional readiness missing capture bisect check: ${JSON.stringify(finalReadiness.checks)}`);
  assert(finalReadiness.harCoverage?.entryCount >= 1, `professional readiness missing HAR coverage summary: ${JSON.stringify(finalReadiness.harCoverage)}`);
  assert(typeof finalReadiness.harCoverage?.totalTiming?.ratio === "number", `professional readiness missing HAR timing coverage: ${JSON.stringify(finalReadiness.harCoverage)}`);
  ...
  assert(finalReadiness.latestResearchPackHandoff?.inspect?.tool === "browser_artifact_inspect", "professional readiness missing latest handoff inspect route");
  ...
  assert(finalReadiness.f12NavigationDrilldowns?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), `professional readiness missing F12 navigation drilldown: ${JSON.stringify(finalReadiness.f12NavigationDrilldowns)}`);
  assert(finalReadiness.researchPackDrilldowns?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), `professional readiness missing research-pack request drilldown: ${JSON.stringify(finalReadiness.researchPackDrilldowns)}`);
  ...
  assert(finalReadiness.recommendedRoute?.some((step) => step.tool === "browser_artifact_index"), "professional readiness missing artifact index in recommended route");
  assert(finalReadiness.artifactDrilldowns?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path), "professional readiness missing artifact drilldown route");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect"), "professional readiness missing handoff inspect next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.harCompletenessPath), "professional readiness missing HAR completeness artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_trace_query" && entry.input?.tracePath === pack.summary.tracePath), "professional readiness missing trace query next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.applicationExportPath), "professional readiness missing Application export artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.evidenceBundlePath), "professional readiness missing evidence bundle artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.drilldownPlanPath), "professional readiness missing drilldown plan artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.evidenceManifestPath), "professional readiness missing evidence manifest artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.correlationGraphPath), "professional readiness missing correlation graph artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.authBoundaryReportPath), "professional readiness missing auth boundary artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.workerFrameReportPath), "professional readiness missing worker/frame artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.f12NavigationPath), "professional readiness missing F12 navigation artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_inspect" && entry.input?.path === pack.summary.firstF12RequestDetailPath), "professional readiness missing first F12 request-detail artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "profile_request_detail" && entry.input?.requestId), "professional readiness missing research-pack request-detail next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "browser_artifact_read" && entry.input?.path), "professional readiness missing artifact-read next action");
  assert(pack.drilldownPlan?.drilldowns?.some((entry) => entry.tool === "profile_request_detail"), "professional pack missing request-detail drilldown");
  assert(pack.drilldownPlan?.planPath === pack.summary.drilldownPlanPath, "professional pack drilldown path mismatch");
  const requestDetailStep = pack.drilldownPlan.drilldowns.find((entry) => entry.tool === "profile_request_detail");
  ...
  const replayStep = pack.drilldownPlan.drilldowns.find((entry) => entry.tool === "profile_request_replay_batch");
  ...
  const traceStep = pack.drilldownPlan.drilldowns.find((entry) => entry.tool === "browser_trace_query");
  ...
  assert(traceDrilldown.recommendedDrilldowns?.some((entry) => entry.tool === "browser_chrome_trace"), "professional trace drilldown missing fresh trace recommendation");
  const handoffPreview = await callTool(baseUrl, "browser_artifact_read", {
    profile: "professional",
  ...
  const drilldownPreview = await callTool(baseUrl, "browser_artifact_inspect", {
    profile: "professional",
  ...
  assert(drilldownPreview.previewText?.includes("profile_request_detail"), "professional drilldown preview missing request detail route");
  const handoffInspect = await callTool(baseUrl, "browser_artifact_inspect", {
    profile: "professional",
    path: pack.summary.researchPackPath,
    maxBytes: 300000,
  });
  ...
  assert(handoffInspect.researchPackHandoff?.panelRoutes?.network?.some((step) => step.tool === "profile_request_detail"), "professional handoff inspect missing network panel route");
  ...
  const parity = await callTool(baseUrl, "browser_f12_parity_matrix", { profile: "professional" });
  ...
  await callTool(baseUrl, "browser_capture_start", {
    profile: "professional",
    clear: true,
  ...
  await sleep(300);
  const network = await callTool(baseUrl, "profile_traffic_summary", {
    profile: "professional",
    limit: 50,
  });
  ...
  const frameTree = await callTool(baseUrl, "browser_frame_tree", {
    profile: "professional",
  });
  assert(JSON.stringify(frameTree).includes("shadow"), "professional fixture missing shadow/frame boundary evidence");

  const sourceMaps = await callTool(baseUrl, "browser_source_map_sources", {
    profile: "professional",
  ...
  const sourceSearch = await callTool(baseUrl, "browser_sources_search", {
    profile: "professional",
    query: "loadProfile",
    reload: true,
    ignoreCache: true,
  ...
  assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_get"), "professional source search missing source_get drilldown");
  assert(sourceSearch.recommendedDrilldowns?.some((entry) => entry.tool === "browser_source_pretty_print"), "professional source search missing pretty-print drilldown");
  const debuggerProbe = await callTool(baseUrl, "browser_debugger_control", {
    profile: "professional",
  ...
  const artifactIndex = await callTool(baseUrl, "browser_artifact_index", {
    profile: "professional",
    maxFiles: 100,
  });
