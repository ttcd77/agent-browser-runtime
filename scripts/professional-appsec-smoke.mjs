...
try {
  ...
  assert(finalReadiness.evidenceEntrypoints?.correlationGraph?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing correlation graph entrypoint: ${JSON.stringify(finalReadiness.evidenceEntrypoints)}`);
  ...
  assert(finalReadiness.routeArtifacts?.workerFrameBoundary?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing compact worker/frame route artifact: ${JSON.stringify(finalReadiness.routeArtifacts)}`);
  ...
  assert(finalReadiness.routeSummary?.evidenceBundleArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing evidence bundle artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.drilldownPlanArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing drilldown plan artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.evidenceManifestArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing evidence manifest artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.correlationGraphArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing correlation graph artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.authBoundaryArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing auth boundary artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.routeSummary?.workerFrameArtifact?.inspect?.tool === "devtools_artifact_inspect", `professional readiness missing worker/frame artifact inspect route: ${JSON.stringify(finalReadiness.routeSummary)}`);
  ...
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.evidenceBundlePath), "professional readiness missing evidence bundle artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.drilldownPlanPath), "professional readiness missing drilldown plan artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.evidenceManifestPath), "professional readiness missing evidence manifest artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.correlationGraphPath), "professional readiness missing correlation graph artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.authBoundaryReportPath), "professional readiness missing auth boundary artifact next action");
  assert(finalReadiness.nextActions?.some((entry) => entry.tool === "devtools_artifact_inspect" && entry.input?.path === pack.summary.workerFrameReportPath), "professional readiness missing worker/frame artifact next action");
