...
- Storage: localStorage, sessionStorage, document-visible cookies, backend cookie API/CDP cookies, cookie security summaries, origin/storage-key/quota evidence, explicit storage-boundary summaries with quota usage breakdowns, Storage Buckets support/bucket summaries where the browser exposes them, cookie partition metadata summaries where Chrome exposes them, IndexedDB database/object-store/index lists, IndexedDB sample records and paged record reads, CacheStorage cache/entry lists, Cache Storage request/response metadata and body reads, Service Worker registrations, Service Worker script/status detail, Service Worker target summary, CacheStorage summary/detail, and Application panel JSON export.
