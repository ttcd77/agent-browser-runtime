...
| Plugin | Agent tool pack | Tools exposed to an AI agent inside a plugin host. |
...
- browser identity reservation needs no external service,
...
- a plugin host has loaded the plugin in a real gateway,
...
- Local storage fixture now records document-visible cookie names alongside backend cookie metadata so agents can see browser/API visibility differences.
