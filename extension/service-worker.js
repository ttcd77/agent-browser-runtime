...
chrome.runtime.onMessage.addListener((message) => {
  if (message?.type === "agent_browser_page_seen") connectBridge();
  return false;
});
...
const SYSTEM_URL_PREFIXES = ["chrome-extension://", "chrome://", "devtools://", "about:"];

function isAttachableUrl(url) {
  if (!url) return false;
  const u = String(url);
  for (const prefix of SYSTEM_URL_PREFIXES) {
...
async function getTargetTab(params = {}) {
  ...
  if (active?.id && isAttachableUrl(active.url)) return active;
  // Active tab is a system/extension URL — fall back to most-recently-accessed http/https tab
  const allTabs = await tabsQuery({});
  const attachable = allTabs
    .filter((t) => t.id && isAttachableUrl(t.url))
    .sort((a, b) => (b.lastAccessed || 0) - (a.lastAccessed || 0));
  if (attachable.length > 0) return attachable[0];
  // No attachable tab exists. Inert reads (status/list) pass autoCreate:false to
  // get the fail-loud error. Real work (snapshot/click/eval/...) lets the
  // extension create a blank tab inline so a fresh Chrome — no human tab open —
  // never blocks the agent (it can navigate the blank tab itself).
  if (params.autoCreate === false) {
    throw new Error("no attachable tab found (all open tabs are chrome:// or extension pages)");
  }
  const created = await chrome.tabs.create({ url: "about:blank", active: false });
  if (!created?.id) throw new Error("failed to auto-create attachable tab");
  return created;
}
...
async function executeCommand(command, params) {
  switch (command) {
    ...
    case "chrome_tab_close":
      return await chromeTabClose(params);
    case "chrome_open":
    ...
    case "chrome_hover":
      return await chromeHover(params);
    case "chrome_double_click":
      return await chromeDoubleClick(params);
    case "chrome_drag":
      return await chromeDrag(params);
    case "chrome_type":
      return await chromeType(params);
    case "chrome_press":
      return await chromePress(params);
    case "chrome_select":
      return await chromeSelect(params);
    case "chrome_wait":
      return await chromeWait(params);
    case "chrome_upload":
      return await chromeUpload(params);
    case "chrome_scroll":
      return await chromeScroll(params);
    case "chrome_eval":
      return await chromeEval(params);
    case "chrome_adopt_tab":
      return await chromeAdoptTab(params);
    case "chrome_resume_profile":
      return await chromeResumeProfile(params);
    case "chrome_auth_bootstrap":
      return await chromeAuthBootstrap(params);
    ...
    case "chrome_cookies_set":
      return await chromeCookiesSet(params);
    case "chrome_cookies_get":
      return await chromeCookiesGet(params);
...
async function chromeStatus() {
  const tabs = await tabsQuery({});
  // Inert read: never side-effect a tab open just to report status.
  let active = null;
  try {
    active = await getTargetTab({ autoCreate: false });
  } catch {
    active = null;
  }
  return {
...
async function chromeTabClose(params = {}) {
  const ids = [];
  if (params.tabId !== undefined && params.tabId !== null) ids.push(Number(params.tabId));
  if (Array.isArray(params.tabIds)) {
    for (const t of params.tabIds) ids.push(Number(t));
  }
  if (!ids.length) {
    return { ok: false, error: "tabId or tabIds required" };
  }
  const closed = [];
  const errors = [];
  for (const id of ids) {
    try {
      ...
      errors.push({ tabId: id, error: String(e?.message || e) });
    }
  }
  return { ok: errors.length === 0, closedCount: closed.length, closed, ...(errors.length ? { errors } : {}) };
}
...
async function chromeSnapshot(params) {
  ...
  // Extension returns the full text unconditionally. The bridge (Node.js process)
  // decides whether to inline or spill to disk based on TEXT_INLINE_THRESHOLD.
  // maxTextLength is accepted but intentionally unused — kept in signature for
  // backwards compat so old callers don't error on an unknown param.
...
async function chromeScreenshot(params) {
  // Use CDP Page.captureScreenshot instead of chrome.tabs.captureVisibleTab so
  // we don't have to force the tab active (which breaks background silent ops).
  // CDP returns inline base64 without a data-URL prefix, so we add it.
  const { tab, target } = await ensureDevtoolsAttached(params);
  const format = params.format === "jpeg" ? "jpeg" : "png";
  const screenshotParams = { format };
  if (format === "jpeg" && params.quality) screenshotParams.quality = Number(params.quality);
...
async function chromeHover(params) {
  const tab = await getTargetTab(params);
  const result = await runInTab(tab.id, ({ selector, text, x, y, frameIndexes }) => {
    function findElement() {
      ...
      return Array.from(doc.querySelectorAll("button,a,input,textarea,select,[role=button],label,summary"))
        .find((node) => (node.innerText || node.value || node.getAttribute("aria-label") || "").toLowerCase().includes(wanted)) || null;
    }
    ...
    const clientX = typeof x === "number" ? x : rect.left + rect.width / 2;
    const clientY = typeof y === "number" ? y : rect.top + rect.height / 2;
    for (const type of ["mouseover", "mouseenter", "mousemove"]) {
      el.dispatchEvent(new MouseEvent(type, { bubbles: true, clientX, clientY }));
    }
    return { ok: true, mode: typeof x === "number" ? "coordinates" : (selector ? "selector" : "text"), x: clientX, y: clientY };
  }, [{ ...params, frameIndexes: frameIndexesFromOptions(params) }]);
  return { tab: pickTab(tab), ...result };
}

async function chromeDoubleClick(params) {
  const tab = await getTargetTab(params);
  const result = await runInTab(tab.id, ({ selector, text, x, y, frameIndexes }) => {
    function findElement() {
      if (typeof x === "number" && typeof y === "number") return document.elementFromPoint(x, y);
    ...
    for (const clickCount of [1, 2]) {
      el.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, clientX, clientY, button: 0, detail: clickCount }));
      el.dispatchEvent(new MouseEvent("mouseup", { bubbles: true, clientX, clientY, button: 0, detail: clickCount }));
      el.dispatchEvent(new MouseEvent("click", { bubbles: true, clientX, clientY, button: 0, detail: clickCount }));
    }
    el.dispatchEvent(new MouseEvent("dblclick", { bubbles: true, clientX, clientY, button: 0, detail: 2 }));
    return { ok: true, mode: typeof x === "number" ? "coordinates" : (selector ? "selector" : "text"), x: clientX, y: clientY };
  }, [{ ...params, frameIndexes: frameIndexesFromOptions(params) }]);
  return { tab: pickTab(tab), ...result };
}

async function chromeDrag(params) {
  const tab = await getTargetTab(params);
  const result = await runInTab(tab.id, (options) => {
    function pointFor(selector, text, x, y, frameIndexes) {
      if (typeof x === "number" && typeof y === "number") return { ok: true, x, y, element: document.elementFromPoint(x, y) };
      let doc = document;
      for (const index of frameIndexes || []) {
        const frames = Array.from(doc.querySelectorAll("iframe,frame"));
        const frame = frames[index];
        ...
        doc = frame.contentDocument || frame.contentWindow?.document;
        if (!doc) return { ok: false, error: "frame_inaccessible" };
      }
      let el = selector ? doc.querySelector(selector) : null;
      if (!el && text) {
        const wanted = String(text || "").toLowerCase();
        el = Array.from(doc.querySelectorAll("button,a,input,textarea,select,[role=button],label,summary,*"))
          .find((node) => (node.innerText || node.value || node.getAttribute?.("aria-label") || "").toLowerCase().includes(wanted)) || null;
      }
      if (!el) return { ok: false, error: "element_not_found" };
      el.scrollIntoView?.({ block: "center", inline: "center" });
      const rect = el.getBoundingClientRect();
      return { ok: true, x: rect.left + rect.width / 2, y: rect.top + rect.height / 2, element: el };
    }
    const source = pointFor(options.selector, options.text, options.x, options.y, options.frameIndexes);
    if (!source.ok) return { ok: false, error: `source_${source.error || "point_failed"}` };
    const targetSelector = options.targetSelector || options.toSelector;
    const targetText = options.targetText || options.toText;
    let target = null;
    if (typeof options.toX === "number" && typeof options.toY === "number") {
      target = { ok: true, x: options.toX, y: options.toY, element: document.elementFromPoint(options.toX, options.toY) };
    } else if (typeof options.deltaX === "number" || typeof options.deltaY === "number") {
      target = { ok: true, x: source.x + Number(options.deltaX || 0), y: source.y + Number(options.deltaY || 0), element: document.elementFromPoint(source.x + Number(options.deltaX || 0), source.y + Number(options.deltaY || 0)) };
    } else {
      target = pointFor(targetSelector, targetText, null, null, options.targetFrameIndexes || []);
    }
    if (!target.ok) return { ok: false, error: `target_${target.error || "point_failed"}` };
    const sourceEl = source.element || document.elementFromPoint(source.x, source.y);
    const targetEl = target.element || document.elementFromPoint(target.x, target.y) || sourceEl;
    sourceEl?.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, clientX: source.x, clientY: source.y, button: 0 }));
    sourceEl?.dispatchEvent(new MouseEvent("dragstart", { bubbles: true, clientX: source.x, clientY: source.y }));
    targetEl?.dispatchEvent(new MouseEvent("mousemove", { bubbles: true, clientX: target.x, clientY: target.y, button: 0 }));
    targetEl?.dispatchEvent(new MouseEvent("dragover", { bubbles: true, clientX: target.x, clientY: target.y }));
    targetEl?.dispatchEvent(new MouseEvent("drop", { bubbles: true, clientX: target.x, clientY: target.y }));
    targetEl?.dispatchEvent(new MouseEvent("mouseup", { bubbles: true, clientX: target.x, clientY: target.y, button: 0 }));
    return { ok: true, from: { x: source.x, y: source.y }, to: { x: target.x, y: target.y } };
  }, [{ ...params, frameIndexes: frameIndexesFromOptions(params) }]);
  return { tab: pickTab(tab), ...result };
}
...
async function chromeType(params) {
  ...
  const result = await runInTab(tab.id, ({ selector, text, clear, pressEnter, framePath, frameIndexes }) => {
    ...
    el.scrollIntoView({ block: "center", inline: "center" });
    el.focus();
    // Use native prototype setter so React/Vue _valueTracker sees a real change.
    // Plain `el.value = x` writes through the framework-overridden setter and
    // React thinks nothing changed, so onChange never fires.
    const isContentEditable = el.isContentEditable && !("value" in el);
    if (isContentEditable) {
      ...
      const setValue = (next) => { if (nativeSetter) nativeSetter.call(el, next); else el.value = next; };
      if (clear !== false) {
        setValue("");
        el.dispatchEvent(new InputEvent("input", { bubbles: true, inputType: "deleteContentBackward", data: null }));
      }
      setValue((el.value || "") + String(text || ""));
    }
...
async function chromePress(params) {
  const tab = await getTargetTab(params);
  if (!params.key) throw new Error("key is required");
  const result = await runInTab(tab.id, ({ selector, key, frameIndexes }) => {
    function parseCombo(combo) {
      ...
      for (const part of parts) {
        ...
        if (["ctrl", "control"].includes(normalized)) ctrl = true;
        else if (["alt", "option"].includes(normalized)) alt = true;
        else if (normalized === "shift") shift = true;
        else if (["meta", "cmd", "command"].includes(normalized)) meta = true;
      }
      ...
      const spec = named[lower] || {
        key: rawKey.length === 1 ? (shift ? rawKey.toUpperCase() : rawKey) : rawKey,
        code: rawKey.length === 1 && /[a-z0-9]/i.test(rawKey) ? (/^[0-9]$/.test(rawKey) ? `Digit${rawKey}` : `Key${rawKey.toUpperCase()}`) : rawKey,
      };
      return { ...spec, ctrl, alt, shift, meta };
    }
    let doc = document;
    for (const index of frameIndexes || []) {
      const frames = Array.from(doc.querySelectorAll("iframe,frame"));
    ...
    const target = selector ? doc.querySelector(selector) : doc.activeElement || doc.body;
    if (!target) return { ok: false, error: selector ? "selector_not_found" : "no_active_element" };
    target.focus?.();
    const spec = parseCombo(key);
    const init = { bubbles: true, cancelable: true, key: spec.key, code: spec.code, ctrlKey: spec.ctrl, altKey: spec.alt, shiftKey: spec.shift, metaKey: spec.meta };
    target.dispatchEvent(new KeyboardEvent("keydown", init));
    target.dispatchEvent(new KeyboardEvent("keyup", init));
    return { ok: true, key, dispatchedKey: spec.key, code: spec.code, selector: selector || null };
  }, [{ ...params, frameIndexes: frameIndexesFromOptions(params) }]);
  return { tab: pickTab(tab), ...result };
}

async function chromeSelect(params) {
  const tab = await getTargetTab(params);
  if (!params.selector) throw new Error("selector is required");
  const result = await runInTab(tab.id, ({ selector, value, label, index, checked }) => {
    ...
    if (tag === "select") {
      const optionList = Array.from(el.options || []);
      let option = null;
      if (value != null) option = optionList.find((item) => item.value === String(value));
      if (!option && label != null) option = optionList.find((item) => item.textContent?.trim() === String(label));
      if (!option && typeof index === "number") option = optionList[index] || null;
      if (!option) return { ok: false, error: "option_not_found", selector, value: value ?? null, label: label ?? null, index: index ?? null };
      el.value = option.value;
      fire();
      return { ok: true, selector, control: "select", value: el.value, label: option.textContent?.trim() || "" };
    }
    if (type === "checkbox" || type === "radio") {
      const desired = typeof checked === "boolean" ? checked : true;
      if (Boolean(el.checked) !== desired) el.click();
      fire();
      return { ok: true, selector, control: type, checked: Boolean(el.checked), value: el.value || "" };
    }
    return { ok: false, error: "unsupported_control", selector, tag, type };
  }, [params]);
  return { tab: pickTab(tab), ...result };
}

async function chromeWait(params) {
  const tab = await getTargetTab(params);
  const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 10_000;
  const pollMs = typeof params.pollMs === "number" ? params.pollMs : 250;
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    const page = await runInTab(tab.id, (inner) => {
      const visible = (el) => {
        if (!el) return false;
      ...
      const el = inner.selector ? document.querySelector(inner.selector) : null;
      const selectorAttached = Boolean(el);
      const selectorVisible = visible(el);
      const textFound = inner.text ? Boolean(document.body?.innerText?.includes(inner.text)) : false;
      const urlFound = inner.urlContains ? location.href.includes(inner.urlContains) : false;
      let ok = true;
      if (inner.selector) {
        if (inner.state === "attached") ok = selectorAttached;
        else if (inner.state === "hidden") ok = selectorAttached && !selectorVisible;
        else if (inner.state === "detached") ok = !selectorAttached;
        else ok = selectorVisible;
      }
      if (inner.text) ok = ok && textFound;
      if (inner.urlContains) ok = ok && urlFound;
      return { ok, url: location.href, selectorAttached, selectorVisible, textFound, urlFound, state: inner.state };
    }, [condition]);
    const networkRows = [...(session.requests?.values?.() || [])].filter((entry) => {
      if (!condition.requestUrlContains) return false;
      if (!String(entry.url || "").includes(condition.requestUrlContains)) return false;
      if (condition.requestMethod && String(entry.method || "").toUpperCase() !== condition.requestMethod) return false;
      if (typeof condition.requestStatus === "number" && Number(entry.status) !== condition.requestStatus) return false;
      return true;
    });
    const latest = networkRows[networkRows.length - 1] || null;
    last = {
      ...page,
      networkFound: condition.requestUrlContains ? Boolean(latest) : true,
      networkRequest: latest ? { requestId: latest.requestId || null, url: latest.url || null, method: latest.method || null, status: latest.status ?? null, failed: Boolean(latest.failed) } : null,
      networkMatchCount: networkRows.length,
    };
    if (condition.requestUrlContains) last.ok = Boolean(last.ok && last.networkFound);
    if (last.ok) return { ok: true, tab: pickTab(tab), waitedMs: Date.now() - startedAt, condition, observation: last };
    await delay(pollMs);
  }
  return { ok: false, error: "timeout", tab: pickTab(tab), waitedMs: Date.now() - startedAt, condition, lastObservation: last };
}

async function chromeUpload(params) {
  if (!params.selector) throw new Error("selector is required");
  const files = [
    ...(Array.isArray(params.files) ? params.files.map(String) : []),
    ...(params.file ? [String(params.file)] : []),
  ];
  if (files.length === 0) throw new Error("file or files is required");
  const { tab, target } = await ensureDevtoolsAttached(params);
  const documentNode = await chromeDebuggerSendCommand(target, "DOM.getDocument", { depth: -1, pierce: true });
  const query = await chromeDebuggerSendCommand(target, "DOM.querySelector", { nodeId: documentNode.root.nodeId, selector: String(params.selector) });
  if (!query.nodeId) return { ok: false, tab: pickTab(tab), error: "selector_not_found", selector: params.selector };
  await chromeDebuggerSendCommand(target, "DOM.setFileInputFiles", { nodeId: query.nodeId, files });
  const verify = await chromeDebuggerSendCommand(target, "Runtime.evaluate", {
    expression: `(() => {
      const el = document.querySelector(${JSON.stringify(String(params.selector))});
      return el && el.files ? { fileCount: el.files.length, names: Array.from(el.files).map((f) => f.name) } : null;
    })()`,
    returnByValue: true,
    awaitPromise: true,
  });
  return { ok: true, tab: pickTab(tab), selector: params.selector, files, result: verify.result?.value ?? null };
}

async function chromeAdoptTab(params) {
  const tab = await getTargetTab(params);
  return {
    ok: true,
    backend: "personal-chrome",
    tab: pickTab(tab),
    profileSemantic: "personal-browser-profile",
    note: "Personal Chrome uses the operator's live browser profile; no managed profile registry entry is created.",
  };
}

async function chromeResumeProfile(params) {
  const tab = await getTargetTab(params);
  return {
    ok: true,
    backend: "personal-chrome",
    tab: pickTab(tab),
    profileSemantic: "personal-browser-profile",
    note: "Personal Chrome continuity is the real Chrome profile and current tabs.",
  };
}

async function chromeAuthBootstrap(params) {
  const tab = await getTargetTab(params);
  const snapshot = await chromeSnapshot({ ...params, tabId: tab.id }).catch(() => null);
  return {
    ok: true,
    backend: "personal-chrome",
    tab: pickTab(tab),
    notApplicable: true,
    reason: "Personal Chrome auth bootstrap is the visible real browser session; the operator completes auth directly in Chrome.",
    snapshot,
  };
}
...
async function chromeEval(params) {
  ...
  // Use CDP Runtime.evaluate so execution happens in V8 directly — bypasses page CSP
  // which would block Function() constructor used by the old runInTab approach.
  const { tab, target } = await ensureDevtoolsAttached(params);
...
function clearCapturedSessionData(session) {
  ...
  session.eventSources = [];
  session.scripts = new Map();
  session.debuggerEvents = [];
  session.paused = null;
}
...
async function chromeCookiesSet(params) {
  const cookies = Array.isArray(params?.cookies) ? params.cookies : [];
  if (!cookies.length) {
    return { ok: false, error: "cookies array is empty or missing" };
  }
  const results = [];
  const errors = [];
  for (const cookie of cookies) {
    try {
      const details = {};
      if (typeof cookie.url === "string") details.url = cookie.url;
      if (typeof cookie.domain === "string") details.domain = cookie.domain;
      if (typeof cookie.path === "string") details.path = cookie.path;
      if (typeof cookie.name === "string") details.name = cookie.name;
      if (typeof cookie.value === "string") details.value = cookie.value;
      ...
      results.push({ name: cookie.name, ok: result !== null, domain: result?.domain ?? null });
    } catch (error) {
      errors.push({ name: cookie.name, error: String(error?.message || error) });
    }
  }
  ...
  return {
    ...
    ...(errors.length ? { perCookieErrors: errors } : {}),
    results,
  };
}

async function chromeCookiesGet(params) {
  const details = {};
  ...
  const cookies = await chrome.cookies.getAll(details).catch((error) => {
...
async function chromeElementsSnapshot(params) {
  ...
  const page = await runInTab(tab.id, ({ maxNodes, maxDepth, selector }) => {
    ...
    const SKIP_TAGS = new Set(["STYLE", "SCRIPT", "SVG", "NOSCRIPT", "TEMPLATE", "HEAD"]);
...
async function chromeDomSearch(params) {
  ...
  await chromeDebuggerSendCommand(target, "DOM.enable").catch((error) => {
    pushUnique(session.warnings, `DOM.enable failed: ${String(error?.message || error)}`);
  });
